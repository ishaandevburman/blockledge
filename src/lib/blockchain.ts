...
import Blockledge from '../../public/abis/Blockledge.json'
import BlockledgeLocal from '../../abis/Blockledge.json'
...
export class AppBlockchain {
    ...
    supportedNetworks = [NetwokIds.sepolia];
    ...
    private async initializeContract(onError: (err: any) => void) {
        if (process.env.NEXT_PUBLIC_MODE === 'DEV') {
            return this.initializeContractLocal().catch((err) => {
                console.error('Error initializing contract locally:', err);
                onError(err);
            });
        }
        if (process.env.NEXT_PUBLIC_MODE === 'PROD') {
            return this.initializeContractRemote().catch((err) => {
                console.error('Error initializing contract remotely:', err);
                onError(err);
            });
        }
    }
    ...
    private async initializeContractLocal() {
        ...
        const networkData = (BlockledgeLocal as any).networks[networkId];
        if (networkData) {
            this.contract = new this.web3.eth.Contract(
                (BlockledgeLocal as any).abi,
                networkData.address
            )
        }
    }
